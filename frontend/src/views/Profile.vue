<template>
  <div class="page-wrapper" :class="getThemeClass">
    ...
    <!-- 3. Темная тема (Звезды) -->
    <div v-if="currentTheme === 'theme_dark'" class="dark-bg-stars">
              ...
              <!-- ИНВЕНТАРЬ -->
               ...
               <div v-if="element === 'info'" class="glass-card card-accent-purple">
                   ...
                   <a-form v-else layout="vertical" class="modern-form">
                       ...
                       <a-form-item label="О себе" style="margin-top:15px">
...
export default {
  ...
  methods: {
    ...
    // === ИСПРАВЛЕННОЕ СОХРАНЕНИЕ БЕЙДЖЕЙ ===
    async toggleEquip(itemId, type) {
        ...
        if (type === 'badge') {
             // 1. Копируем текущий массив, чтобы не мутировать напрямую до проверки
             let badges = [...(this.profile.equipped_rewards.badges || [])];

             if (badges.includes(itemId)) {
                 badges = badges.filter(b => b !== itemId); // Удаляем
             } else {
                 if (badges.length >= 2) {
                     message.warn('Максимум 2 бейджа!');
                     return;
                 }
                 badges.push(itemId); // Добавляем
             }

             // 2. Обновляем локальное состояние
             this.profile.equipped_rewards.badges = badges;

             // 3. ОТПРАВЛЯЕМ МАССИВ НА СЕРВЕР (Fix: отправляем список, а не один ID)
             await this.syncEquip('badge', badges);
             return;
        }
    ...
    async loadData() {
        try {
            ...
            if (!this.profile.equipped_rewards) this.profile.equipped_rewards = {};
            // ВАЖНО: Инициализируем массив бейджей, если его нет
...
/* =======================================================
   БАЗОВЫЕ СТИЛИ
   ======================================================= */
/* Исправлена белая полоса - min-height 100vh глобально */
.page-wrapper { min-height: 100vh; background: #f3f4f6; display: flex; justify-content: center; padding: 30px 20px; position: relative; overflow-x: hidden; font-family: 'Inter', sans-serif; }
...
/* Загрузка фото */
.avatar-upload-pos { position: absolute; bottom: 10px; right: 10px; z-index: 50; }
.btn-mini-edit { width: 32px; height: 32px; border-radius: 50%; background: #2563eb; color: white; border: 2px solid white; cursor: pointer; box-shadow: 0 4px 10px rgba(0,0,0,0.2); display: flex; align-items: center; justify-content: center; transition: 0.2s; }
...
/* Инфо поля */
.info-row { display: flex; justify-content: space-between; align-items: center; border-bottom: 1px dashed #e5e7eb; padding: 10px 0; }
...
.roadmap-widget-content { text-align: center; padding: 10px 0; }
.roadmap-title { font-size: 1rem; font-weight: 700; color: #374151; margin-bottom: 12px; }
.circle-wrapper { margin: 0 auto 12px; width: 100px; position: relative; }
...
/* === ФИКС 2: ПРОГРЕСС БАР ВОКРУГ АВАТАРКИ (ЦВЕТА) === */
...
/* 3.2 ТЕМНАЯ ТЕМА (НОВЫЕ ПРАВКИ) */
.theme-dark-mode { background-color: #111827 !important; color: #e5e7eb; }

/* Динамика: Звезды */
.dark-bg-stars { position: fixed; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; }
...
/* Исправление цвета текста на белый/светло-серый */
...
.theme-dark-mode .btn-glass-edit:hover, .theme-dark-mode .settings-btn:hover {
    background: #4b5563 !important; color: white !important;
}
...
.gold-bg-dust { position: fixed; width: 100%; height: 100%; pointer-events: none; }
